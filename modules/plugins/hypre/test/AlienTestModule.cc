...
AlienTestModule::test()
{
  ...
  {
    Alien::Hypre::LinearAlgebra algebra2;
    algebra2.copy(vectorB, vectorX); // copy b in x using hypre
  ...
  Alien::MatrixData matrixA(space,
      m_mdist); // local matrix for exact measure without side effect (however, you can
                // reuse a matrix with several builder)
  if (m_vect_size == 0) {
        ...
        if (m_vect_size == 1)
          for (Integer loop = 0; loop < options()->repeatLoop(); ++loop) {
            ...
            Alien::DirectMatrixBuilder builder(
                matrixA, Alien::DirectMatrixOptions::eResetValues);
            if (loop == 0) {
              profile_stat.start();
      ...
      case AlienTestOptionTypes::ProfiledBuilder: {
        ...
        for (Integer loop = 0; loop < options()->repeatLoop(); ++loop) {
          ...
          if (m_vect_size == 0) {
            ...
            Alien::ProfiledBlockMatrixBuilder builder(matrixA,
...
{
  Alien::LocalBlockVectorWriter b(vectorB);
  ENUMERATE_CELL (icell, m_areaT.own()) {
